...
public class ProcessManager {
    ...
    @Autowired
    private BpkLoader bpkLoader;

    @Autowired
    private BpkReceiver bpkReceiver;
    ...
    private RunningTaskHandler loadBpk(RunningTaskHandler handler) {
        ...
        try {
            bpkReceiver.receive(handler.getTaskDefinition().getBpkPointer(), bpkExtracted);
            Bpk bpk = bpkLoader.load(bpkExtracted);
            handler.setBpk(bpk);
        } catch (IOException e) {
            // FIXME
            e.printStackTrace();
