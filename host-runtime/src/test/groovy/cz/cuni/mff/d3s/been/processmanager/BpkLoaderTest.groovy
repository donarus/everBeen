...
public class BpkLoaderTest extends Specification {
    ...
    def 'loading valid Java BPK'() {
        ...
        when:
            JavaBpk bpk = BpkUtil.loadUnpackedBpk(unpackedBpk)
        then:
            bpk.bpkId.artifactId== "task-example-helloworld"
            bpk.bpkId.groupId == "cz.cuni.mff.d3s.been"
            bpk.bpkId.version == "1.0.0-SNAPSHOT"
    ...
    def 'Java BPK without #missingProperty cannot be loaded'() {
        given:
            ...
            configJson.text = JsonOutput.toJson(input)
        when:
            BpkUtil.loadUnpackedBpk(unpackedBpk)
        then:
